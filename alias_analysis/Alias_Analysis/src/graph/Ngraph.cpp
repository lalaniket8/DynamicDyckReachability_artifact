...
void Ngraph::construct(string infile_name){
	...
	while(std::getline(infile,line)){
		...
		if(tokens[0] == "e"){
			...
			if(getfield(stoi(tokens[3]))==EPS){
				addedge(getVertex(stoi(tokens[2])),getVertex(stoi(tokens[1])),EPS);	
			}
			continue;
		}
		if(tokens[0] == "v"){
...
Nvertex* Ngraph::getVertex(const int64_t &s){
...
field& Ngraph::getfield(const int64_t &s){
