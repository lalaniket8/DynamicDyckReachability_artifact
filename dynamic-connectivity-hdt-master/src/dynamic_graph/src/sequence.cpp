...
std::pair<int64_t, int64_t> Element::getRoot_optimized(int64_t node) {
  if(root_node_.first == -1){
    if(parent_ == nullptr){
      // Element* current{this};
      root_node_ = id_;
    }else{
      root_node_ = parent_->getRoot_optimized(node);
    }
    // std::cout << "NODE(" << node << ")(" << id_.first << "," << id_.second << ") root_node_:(" << root_node_.first << "," << root_node_.second << ")" << std::endl;
  }
  return root_node_;
}
...
int64_t Element::getSeqRoot(){
  Element* current{this};
  while(children_[Direction::kLeft] != nullptr){
    current = children_[Direction::kLeft];
  }
  int64_t root = current->id_.first;
  return root;
}

void Element::getComponent(std::set<int64_t>* output) const{ //O(size of component)
  if (children_[Direction::kLeft] != nullptr) {
    children_[Direction::kLeft]->getComponent(output);
  }
  if(id_.first == id_.second){
    (*output).insert(id_.first);
  }
  if (children_[Direction::kRight] != nullptr) {
    children_[Direction::kRight]->getComponent(output);
  }
}

std::set<int64_t> Element::getComponent() const{
  const Element* root = GetRoot();
  std::set<int64_t> output;
  root->getComponent(&output);
  return output;
}

bool Element::isRoot() const{
  const Element* current{this};
  return current->parent_ == nullptr;
}

// std::ostream& Element::operator<<(std::ostream& out) {
//   out << "(" << this->id_.first << "," << this->id_.second << ")";
// }

// std::string Element::int64_t_tostring(const int64_t& number) {
...
//   std::string str = "(" + int64_t_tostring(this->id_.first) + "," + int64_t_tostring(this->id_.second) + ")"; 
//   return str;
// } 

void Element::print(){
  Id reprId = GetRoot()->id_;
  std::cout << "[id_:(" << id_.first << "," << id_.second << "); parent:" << (parent_ == nullptr ? "NULL":"NOTNULL") << "; repr:(" << reprId.first << "," << reprId.second << ")]" << std::endl;
}
