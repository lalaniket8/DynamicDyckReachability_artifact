...
std::unordered_map<UndirectedEdge, detail::EdgeInfo, UndirectedEdgeHash> DynamicConnectivity::getEdges(){
  return edges_;
}

std::pair<int64_t, int64_t> DynamicConnectivity::getRoot(int64_t nodeID){
  return spanning_forests_[0].getRoot(nodeID);
}

std::set<int64_t> DynamicConnectivity::getRoots(){
  return spanning_forests_[0].getRoots();
}

std::pair<int64_t, int64_t> DynamicConnectivity::getNode(Vertex node){
  return spanning_forests_[0].getNode(node);
}

std::pair<int64_t, int64_t> DynamicConnectivity::getPred(Vertex node){
  return spanning_forests_[0].getPred(node);
}

std::vector<std::pair<int64_t, int64_t>> DynamicConnectivity::getSequenceOfNode(Vertex node){
  return spanning_forests_[0].getSequenceOfNode(node);
}

std::set<int64_t> DynamicConnectivity::getComponent(Vertex node){
  return spanning_forests_[0].getComponent(node);
}

std::pair<int64_t, int64_t> DynamicConnectivity::getParent(Vertex node){
  return spanning_forests_[0].getParent(node);
}

bool DynamicConnectivity::isRoot(Vertex node){
  return spanning_forests_[0].isRoot(node);
}

//O(nlogn)
std::set<std::pair<int64_t, std::set<int64_t>>> DynamicConnectivity::getComponents(){
  std::set<std::pair<int64_t, std::set<int64_t>>> components;
  std::set<int64_t> roots = spanning_forests_[0].getRoots();//O(nlogn)
  for(const int64_t& iter : roots){
  // for(std::set<int64_t>::iterator iter = roots.begin(); iter != roots.end(); iter++){
    components.insert({iter,getComponent(iter)});//O(size of component)
  }
  return components;
}

int64_t DynamicConnectivity::getPredRoot(int64_t node){
  return spanning_forests_[0].getPredRoot(node);
}

int64_t DynamicConnectivity::getSeqRoot(Vertex node){
  return spanning_forests_[0].getSeqRoot(node);
}

// void DynamicConnectivity::showComponents(){
//   std::cout << "graphHDT->showComponents():" << std::endl;
//   std::set<std::pair<int64_t, std::set<int64_t>>> componentsHDT = this->getComponents();
...
// void DynamicConnectivity::showComponents(std::unordered_map<std::string,int64_t>::iterator NODES_iter_begin, std::unordered_map<std::string,int64_t>::iterator NODES_iter_end){
//   std::cout << "graphHDT->showComponents():" << std::endl;
//   std::set<std::pair<int64_t, std::set<int64_t>>> componentsHDT = this->getComponents();
//   for(std::set<std::pair<int64_t, std::set<int64_t>>>::iterator comp_iter = componentsHDT.begin(); comp_iter != componentsHDT.end(); comp_iter++){
//       std::set<int64_t> component = comp_iter->second;
//       std::string compName = "<NONE>";
//       for(std::unordered_map<std::string,int64_t>::iterator it = NODES_iter_begin; it != NODES_iter_end; it++){
//         if(it->second == comp_iter->first){
//           compName = it->first; break;
//         }
//       }
//       std::cout << comp_iter->first << "(" << compName << "):";
//       for(auto it = component.begin(); it!=component.end(); it++){

//         std::string nodeName = "<NONE>";
//         for(std::unordered_map<std::string,int64_t>::iterator it_NODES = NODES_iter_begin; it_NODES != NODES_iter_end; it_NODES++){
//           if(it_NODES->second == (*it)){
//             nodeName = it_NODES->first; break;
//           }
//         }
...
void DynamicConnectivity::print(){
  spanning_forests_[0].print();
}

void DynamicConnectivity::set_rootnode_null(){
  spanning_forests_[0].set_rootnode_null();
}

std::pair<int64_t, int64_t> DynamicConnectivity::getRoot_optimized(int64_t nodeID){
  return spanning_forests_[0].getRoot_optimized(nodeID);
}
