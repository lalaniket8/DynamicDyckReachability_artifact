...
std::pair<int64_t, int64_t> DynamicForest::getNode(Vertex node){
  return vertices_[node].id_;
}

std::pair<int64_t, int64_t> DynamicForest::getRoot(Vertex node){
  return vertices_[node].GetRepresentative()->id_;
}

std::pair<int64_t, int64_t> DynamicForest::getPred(Vertex node){
  Element* el = vertices_[node].GetPredecessor();
  if(el == NULL){
    return {-2,-2};
  }
  return el->id_;  
...
std::vector<std::pair<int64_t, int64_t>> DynamicForest::getSequenceOfNode(Vertex node){
  return vertices_[node].SequenceIds();
}

std::pair<int64_t, int64_t> DynamicForest::getParent(Vertex node){
  return vertices_[node].parent_->id_;
}

std::set<int64_t> DynamicForest::getComponent(Vertex node){
  return vertices_[node].getComponent();
}

bool DynamicForest::isRoot(Vertex node){
  return vertices_[node].GetPredecessor() == NULL;
}

int64_t DynamicForest::getPredRoot(Vertex node){//O(logn*logn) time
  Element* root = &vertices_[node];
  while(root->GetPredecessor() != NULL){
    root = root->GetPredecessor();
  }
  return root->id_.first;
}

std::set<int64_t> DynamicForest::getRoots(){//O(nlogn)
  std::set<int64_t> roots = {};
  for(std::vector<sequence::Element>::iterator iter = vertices_.begin(); iter != vertices_.end(); iter++){
    // std::set<int64_t> comp = iter->getComponent();
    // std::cout << "(" << iter->id_.first << "," << iter->id_.second << "):" << std::endl;
    roots.insert(getRoot(iter->id_.first).first);
    ...
    // pair<int64_t,int64_t> root = getRoot(*iter);
    // cout << "Root of " << *iter << ":" << root.first << "," << root.second << endl;
    // if(iter->id_.first == iter->id_.second){
      // if(iter->isRoot()){
      //   roots.insert(iter->id_.first);
      // }
    // }
  }
  return roots;
...
int64_t DynamicForest::getSeqRoot(Vertex node){
  return vertices_[node].getSeqRoot();
}

void DynamicForest::print(){
  std::cout << "vertices_:{";
...
void DynamicForest::set_rootnode_null(){

  for(int i=0;i<vertices_.size();i++){
    vertices_[i].root_node_.first = -1;
    vertices_[i].root_node_.second = -1;
  }
  for(int i=0;i<edge_elements_.size();i++){
    edge_elements_[i].root_node_.first = -1;
    edge_elements_[i].root_node_.second = -1;
  }
  for(int i=0;i<free_edge_elements_.size();i++){
    free_edge_elements_[i]->root_node_.first = -1;
    free_edge_elements_[i]->root_node_.second = -1;
  }
}

std::pair<int64_t, int64_t> DynamicForest::getRoot_optimized(Vertex node){
  return vertices_[node].getRoot_optimized(node);
}
