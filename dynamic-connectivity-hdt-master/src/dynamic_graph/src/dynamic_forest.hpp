...
class DynamicForest {
 public:
  ...
  ~DynamicForest();
  std::pair<int64_t, int64_t> getRoot(Vertex node);
  ...
  //  std::vector<sequence::Element> getVertices();
  ...
  int64_t getPredRoot(Vertex node);
  int64_t getSeqRoot(Vertex node);
  void print();
  void set_rootnode_null();
  std::pair<int64_t, int64_t> getRoot_optimized(Vertex node);
 private:
